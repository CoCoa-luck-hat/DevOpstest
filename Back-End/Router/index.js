...
router.post('/register', RegisterValidator, async (req, res) => {
    ...
    try {
        ...
        if (row.length >= 1) {
            ...
            const response = await pool.execute("INSERT INTO `users` (`id_user`, `username`, `email`, `password`, `role`, `created_at`) VALUES (NULL, ?, ?, ?, 'user', CURRENT_TIMESTAMP)", [username, email, password])
            res.status(201).json({
...
router.post('/login', LoginValidator, async (req, res) => {
    ...
    try {
        ...
        if (row.length == 0) {
            ...
            const user = row[0]
            const token = jwt.sign({ id_user: user.id_user, email: user.email, role: user.role }, process.env.TOKEN_KEY, { expiresIn: '1h' })
            res.status(200).json({
